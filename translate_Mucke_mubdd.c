...
static int isThreadFunction(bp_fun_t f) {
    ...
    // begin with thread (or this word in mixed-case)
    if (
        strlen(f->funname) > 6 &&
...
static void print_d_pc(bp_stmt_t p) {           // Print dead pc - the last pc
    printf("&");
    if (p->next_stmt) {
...
static void CopyVariables_ProgramInt(void) {
    ...
    // TRUC's modification
    print_Exemption_CopyVariables();    // program call by itself already copy variables
    printf("| (true\n");
...
static void print_recursive_statement_program_Call(bp_stmt_t p, bp_fun_t fun) {
    ...
    case BP_CALL: {
        ...
        if (t || q) {
            common_error("error in ProgramCall: the list of parameters and arguments are of different length\n");
        }
...
///////////////////////////////////////////////////////////////////////////////////////////
static void print_recursive_SetReturnTS_FromCall_globals(
    bp_stmt_t p, bp_fun_t fun_call, bp_stmt_t p_return, bp_fun_t fun_return, bp_ident_t var, short *head)
{
    bp_ident_t z, y, temp_var;
    bp_idref_t assign_call, list_return_expr;
...
print_Mucke_program(int ord, char *formulaFilename) {
    ...
    printf("bool LessEqualCS(CS r, CS rp)\n");
    printf("r ~+ rp\n");
    printf("(   ( r=0 )\n");
    for (i = 0; i < CS - 1; i++) {
        printf("  | ( r=%d ", i);
    ...
    printf("bool increaseCS(CS r, CS rp)\n");
    printf("r ~+ rp\n");
    printf("( false\n");
    for (i = 0; i < CS - 1; i++)
        printf("  | ( r=%d & rp=%d )\n", i, i + 1);
