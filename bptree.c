...
bp_ident_t
Is_there_var(bp_ident_t p, char* name)
{

	bp_ident_t q;
	for (q = p; q; q = q->next)
		if (strcmp(name, q->varname) == 0) return (q);
...
bp_ident_t
Insert_var(char* name)
{
	bp_ident_t tempvar, tempvar2;

	//allocate the new node for the new variables

	tempvar = alloc_bp_ident();

	tempvar->varname = name;
	bp_declcount++;
	tempvar->index = bp_declcount;
	tempvar->next = NULL;

	//insert the new node at the tail of the list bp_decls
	if (!bp_decls)
	{
		bp_decls = tempvar;
	}
	else
	{	// find the last element of the list bp_decls
		for (tempvar2 = bp_decls; tempvar2->next; tempvar2 = tempvar2->next);
		tempvar2->next = tempvar;
	}
...
signed char bp_insert_decl (bp_tree_t* tree,
                            Name name, short fun, bp_ident_t id)
{
	bp_tree_t tmp;
	signed char bal;
	...
	if ((*tree)->name == name)
	{
		...
		{
			common_error("conflicting declarations");
			return 0;
		}
	...
	common_error("this shouldn't happen");
	return 0;
}

bp_ident_t bp_lookup (bp_tree_t tree, Name name, short fun)
{
	if (!tree) return NULL;
	if (tree->name == name)
	{
...
/*
bp_ident_t bp_find_var (bp_tree_t tree, Name name, short fun)
{
	bp_ident_t res = bp_lookup(tree,name,fun);
	if (!res) common_error("name lookup failure (%s/%d)",
			name_cstring(name),fun);
	return res;
}

bp_ident_t bp_find_decl (Name name, short fun)
{
...
// bp_assign_glob_index (varname) adds a new node in the list of
// global ids, and returns the index assigned to it.
//       - increment bp_declcount
//       - increment bp_maxrets

/*
int bp_assign_glob_index (Name varname)
{
	int retval = 0;
...
int bp_assign_loc_index (Name varname, short fun)
{
	int retval = 0;
...
int bp_assign_parm_index (Name varname, short fun)
{
	if (mc_sepvar == 3)
	{
		char tmp[32];
		sprintf(tmp,"*param%d",bp_declcount+1);
		varname = name_create(tmp);
	}

	return bp_assign_loc_index(varname,fun);
}

int bp_assign_ret_index (Name varname, int i)
{
	int retval = 0;

	retval = bp_numglobals + i;
...
/*
int bp_assign_cb_index (Name varname, short fun, int i)
{
	int retval = 0;
